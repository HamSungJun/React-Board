...
class DB_Machine {
  INSERT_NONFORMAL(document, collection) {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(secret.MongoCollections.formalUsers)
          ...
          .count((err, count) => {
            ...
            if (count >= 1) {
              ...
              db.collection(collection).insertOne(document, (err, result) => {
                ...
                console.log(
                  `Inserted 1 document into ${secret.MongoDB}.${collection}`
                );
                client.close();
  ...
  MOVE_NONFORMAL_TO_FORMAL(unique, base, target) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(base)
          .find({ TIMEID: unique })
          .project({ EMAIL: 1, USERNAME: 1, PW: 1, U_IMG_PATH: 1 })
          .toArray((err, docs) => {
            assert.equal(err, null);
  ...
  SAVE_USER_SHARED_POSTING(doc) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(secret.MongoCollections.sharedPostings).insertOne(
          doc,
          (err, result) => {
            assert.equal(err, null);
            assert.equal(1, result.insertedCount);
  ...
  SAVE_USER_TEMP_DOCUMENT(doc) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(secret.MongoCollections.tempSavedDocs).insertOne(
          doc,
          (err, result) => {
            assert.equal(err, null);
            assert.equal(1, result.insertedCount);
  ...
  GET_USER_TEMP_DOCUMENT_WITHOUT_CONTENT(user) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(secret.MongoCollections.tempSavedDocs)
          .find(
            { AUTHOR: user },
            {
              projection: {
  ...
  POST_REPLY_BY_ID(_id, doc) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(err => {
        ...
        db.collection(secret.MongoCollections.sharedPostings).findOneAndUpdate(
          { _id: new ObjectId(_id) },
          { $push: { POST_REPLY: doc } },
  ...
  POST_EYE_UP_BY_ID(targetArticleId) {
    return new Promise((resolve, reject) => {
      const client = new MongoClient(secret.MongoURL, {
        useNewUrlParser: true
      });

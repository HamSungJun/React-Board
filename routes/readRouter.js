...
readRouter.post('/deleteReply', (req,res) => {
    ...
    let DB_Machine = new MonDB()
    DB_Machine.DELETE_REPLY_BY_ID(req.body.targetArticleId,req.body.targetReplyId).then(response => {
        if(response.status === 1){
            return res.json(response).end()
        }
    })
...
readRouter.post('/recommendUp', (req,res) => {
    ...
    DB_Machine.RECOMMEND_UP_BY_EMAIL(req.body.targetArticleId,req.body.EMAIL).then(response => {
        if(response.status === 1){
            return res.json(response).end()
        }
    })
